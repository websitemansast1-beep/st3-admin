...
// the instant the access token's 15-minute exp has passed, try a silent
// refresh first and only send them to login if that refresh also fails.
// Returns true/false so the init flow below can wait for it before
// loading any page data.
...
// ===== NEW: Add Multiple Vocabulary Words =====
function setVocabMode(mode) {
  const singleMode = document.getElementById('vocab-single-mode');
  const bulkMode = document.getElementById('vocab-bulk-mode');
...
async function addMultipleVocabWords() {
  const textarea = document.getElementById('vocab-bulk-text');
  const bulkText = textarea?.value.trim();
  if (!bulkText) { 
    toast('❌ اكتب الكلمات (كل كلمة في سطر)'); 
    return; 
  }
  const lang = document.getElementById('vocab-bulk-lang')?.value;
  const rate = parseFloat(document.getElementById('vocab-bulk-rate')?.value) || 1;
  ...
  if (!words.length) { 
    toast('❌ لا توجد كلمات للإضافة'); 
    return; 
  }
  
  words.forEach(word => {
    currentVocabWords.push({ text: word, lang, rate });
  });
  
  await saveVocabList_();
  textarea.value = '';
  toast(`✅ تمت إضافة ${words.length} كلمة`);
  setVocabMode('single');
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Wait for requireAuth() to finish (including a silent refresh, if one
  // was needed) before deciding whether to load page data — firing the
  // page's load function in parallel with an in-flight refresh could
  // otherwise let the FIRST api() call race the refresh and 401 before
  // the new token was saved.
